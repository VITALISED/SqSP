...
static void SquirrelCompileError (HSQUIRRELVM sqvm, const SQChar* desc, const SQChar*, SQInteger line, SQInteger column)
{
	PrintError("SQUIRREL ERROR: Line " FMT_INT ", Col " FMT_INT ": %s\n", line, column, desc);
}
...
static bool LoadScript (HSQUIRRELVM sqvm,const char* fileName)
{
	bool	res  = false;
	FILE*	file = NULL;
	SQChar* buf  = NULL;
...
void RegInt (HSQUIRRELVM sqvm,const SQChar* name, SQInteger value, SQBool isConst)
{
	if (isConst)
...
void RegFloat (HSQUIRRELVM sqvm,const SQChar* name, SQFloat value, SQBool isConst)
{
	if (isConst)
		sq_pushconsttable(sqvm);
	else
...
void RegStr (HSQUIRRELVM sqvm,const SQChar* name, const SQChar* value, SQBool isConst)
{
	if (isConst)
		sq_pushconsttable(sqvm);
	else
...
//	sq_setmaxparams(sqvm, maxParams);
	if (SQ_FAILED(sq_newslot(sqvm, -3, SQFalse)))
...
// Convert path to OS's native format (toNative = true)or from OS's native format to *nix format (toNative = false)
static SQInteger ConvPath2 (HSQUIRRELVM sqvm)
{
	const SQChar* path;
	SQBool		  toNative;
...
static SQInteger PathEnv (HSQUIRRELVM sqvm)
{
...
static SQInteger Platform (HSQUIRRELVM sqvm)
{
...
static SQInteger CPUArch (HSQUIRRELVM sqvm)
{
...
static SQInteger StrChar (HSQUIRRELVM sqvm)
{
	SQInteger	  res = 0;
	const SQChar* str;
	SQInteger	  i;
...
static SQInteger SubStr (HSQUIRRELVM sqvm)
{
	bool		  ok = false;
	const SQChar* str;
	SQInteger	  a, b;
...
static SQInteger FilePath (HSQUIRRELVM sqvm)
{
	const SQChar* path;
	sq_getstring(sqvm, 2, &path);
...
static SQInteger FileName (HSQUIRRELVM sqvm)
{
	const SQChar* path;
	SQBool		  ext = SQFalse;
	sq_getstring(sqvm, 2, &path);
...
static SQInteger FileExt (HSQUIRRELVM sqvm)
{
	const SQChar* path;
	sq_getstring(sqvm, 2, &path);
...
//    DIR  - directory;
//    FILE - file.
static SQInteger FileType2 (HSQUIRRELVM sqvm)
{
	const SQChar* path;
	sq_getstring(sqvm, 2, &path);
...
//    %Y - Year in full
static SQInteger StrTime (HSQUIRRELVM sqvm)
{
	SQInteger	  intTime = 0;
...
static SQInteger UtcToLocal (HSQUIRRELVM sqvm)
{
	SQInteger sysTime;
...
static SQInteger LocalToUtc (HSQUIRRELVM sqvm)
{
	SQInteger localTime;
...
// Time should be between 0:0:0 AM and 11:59:59 PM
// Values outside specified range are clamped
static SQInteger MkTime (HSQUIRRELVM sqvm)
{
	SQInteger year,
			  month,
			  day,
...
void Init_Util (HSQUIRRELVM sqvm)
{
#if !defined(SHELL_PLATFORM_WINDOWS)
	...
	RegInt(sqvm,"DIR", FILETYPE_DIR, SQTrue);
	RegInt(sqvm,"FILE", FILETYPE_FILE, SQTrue);
	RegCFunc(sqvm,"convpath", ConvPath2, 3, 0, ". s b");
	RegCFunc(sqvm,"pathenv", PathEnv, -2, 0, ". s");
	RegCFunc(sqvm,"platform", Platform, 1, 0, ".");
	RegCFunc(sqvm,"cpuarch", CPUArch, 1, 0, ".");
	RegCFunc(sqvm,"strchar", StrChar, 3, 0, ". s i");
	RegCFunc(sqvm,"substr", SubStr, 4, 0, ". s i i");
	RegCFunc(sqvm,"filepath", FilePath, 2, 0, ". s");
	RegCFunc(sqvm,"filename", FileName, -2, 3, ". s b");
	RegCFunc(sqvm,"fileext", FileExt, 2, 0, ". s");
	RegCFunc(sqvm,"filetype", FileType2, 2, 0, ". s");
	RegCFunc(sqvm,"strtime", StrTime, -2, 3, ". i s");
	RegCFunc(sqvm,"utctolocal", UtcToLocal, 2, 0, ". i");
	RegCFunc(sqvm,"localtoutc", LocalToUtc, 2, 0, ". i");
	RegCFunc(sqvm,"mktime", MkTime, 7, 0, ". i i i i i i");
